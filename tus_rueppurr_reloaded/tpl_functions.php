...
function tpl_hide_menu() {
  global $ID;

  $pages = split(',;:', tpl_getConf('btl_hide_menu_for_pages'));
...
function tpl_hide_sidebar() {
  global $ID;

  $pages = split(',;:', tpl_getConf('btl_hide_sidebar_for_pages'));
  foreach( $pages as $page ) {
    if( $ID == $page ) {
      return true;
    }
...
function check_namespace($prefix) {
  global $ID;
